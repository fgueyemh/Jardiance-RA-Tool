$(function() {
    ...
    function idleMessage() {
    	...
    	if(currentQuestion.length && $('.cf-idle').length == 0) {
	    	...
	    	if(currentQuestion < 15){
	    		idleCopy = '<p>Encore quelques questions! Continuez pour connaître les résultats de votre évaluation.</p>';
	    	} else {
	    		idleCopy = '<p>Vous avez presque terminé! Continuez et vous recevrez les résultats de votre évaluation. </p>';
	    	}
			...
			idleHtml += idleCopy;
			idleHtml += '<p>Souhaitez-vous continuer l\'évaluation sur le risque de maladie cardiaque ou recommencer au début?</p></div>';
			idleHtml += '<div class="buttons">';
			idleHtml += '<button class="idle" id="idle-1" value="no" score="1"><label for="idle-1">Je veux continuer </label></button>';
			idleHtml += '<button class="idle" id="idle-2" value="yes" score="1"><label for="idle-1">J\'aimerais recommencer</label></button>';
			idleHtml += '</div>';
			idleHtml += '</div>';
    ...
    function awayMessage() {
    	...
    	if(localcurrentQuestion.length && $('.cf-away').length == 0) {
			...
			idleHtml += '<div class="heart-container"><div class="heart"></div><img src="heart-icon.svg" alt=""></div>';
			idleHtml += '<div class="text-container"><p>Rebonjour!</p>';
			idleHtml += "<p>Souhaitez-vous continuer l'évaluation sur le risque de maladie cardiaque ou recommencer au début?</p></div>";
			idleHtml += '<div class="buttons">';
			idleHtml += '<button class="idle" id="idle-1" value="no" score="1"><label for="idle-1">Je veux continuer</label></button>';
			idleHtml += '<button class="idle" id="idle-2" value="yes" score="1"><label for="idle-1">J\'aimerais recommencer </label></button>';
